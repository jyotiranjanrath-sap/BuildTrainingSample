...
class DeployCF {
    ...
    constructor(args) {
        ...
        instance = this;
        instance._startUrl = "";
        instance._validRouter = false;
        instance._option = {};
        instance.failedByBuild = false;
    ...
    deploy() {
        return instance._getProjectInfo()
            ...
            .then(instance._checkRouterVersion)
            .then(function () {
                if (!instance._startUrl || instance.deployArgs.force() || !instance._validRouter) {
            ...
            .catch(error => {
                if (!instance.failedByBuild && instance.deployArgs.logFile()) {
                    fs.writeFileSync(instance.deployArgs.logFile(), error);
                }
    ...
    _prepareMta() {
        return new Promise(function (resolve, reject) {
            ...
            if (instance.deployArgs.logFile()) {
                sCommand = sCommand + ` --logFile ${instance.deployArgs.logFile()}`;
            }
    ...
    _checkRouterVersion() {
        return new Promise(function (resolve, reject) {
            if (instance._startUrl) {
                let sAppName = instance._getName();
                let sCommand = `cf env ${sAppName}-approuter | grep SAP_ROUTER_VERSION`;
                instance._validRouter = false;
                try {
                    console.log(sCommand);
                    let output = execSync(sCommand).toString();
                    console.log(output);
                    if (output && output.indexOf("SAP_ROUTER_VERSION") > -1) {
                        let version = parseInt(output.split(':').pop());
                        if (version && version>=19) {
                            instance._validRouter = true;
                        }
                    }
                    resolve();
                } catch (error) {
